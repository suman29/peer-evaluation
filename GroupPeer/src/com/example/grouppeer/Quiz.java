...
import com.example.grouppeer.R;
		...
		while( true )
		{
			...
			if( packetRcvd.leader_req_packet == true )
			{
				...
				if( packetRcvd.seq_no == PacketSequenceNos.GROUP_SERVER_SEND && lpRecvd.grpNameRequest == true )
				{
					...
					Intent i = new Intent(Quiz.staticAct, com.example.grouppeer.Group_name.class);
				...
				else if(  packetRcvd.seq_no == PacketSequenceNos.SELECTED_LEADERS_SERVER_SEND && lpRecvd.LeadersListBroadcast == true )
				{
					...
					Intent i = new Intent(Quiz.staticAct, com.example.grouppeer.Select_leader.class);
					Quiz.staticAct.startActivity(i);
					Quiz.staticAct.finish();
					break;
				}
...
public class Quiz extends Activity implements OnClickListener {
	...
	//Button skipButton;
	Button leader;
	private String errormessage,groupleader,sessionexpire,denyleader,onlinestudents,rounds,subject,noofleaders,groupmembers;
    ...
    public void onCreate(Bundle savedInstanceState) 
    {
		...
		sock = SocketHandler.normalSocket;
		staticAct = this;
		errormessage=this.getString(R.string.errormessage);
		groupleader=this.getString(R.string.groupleader);
		sessionexpire=this.getString(R.string.sessionexpire);
		denyleader=this.getString(R.string.denyleader);
		onlinestudents=this.getString(R.string.onlinestudents);
		rounds=this.getString(R.string.rounds);
		subject=this.getString(R.string.subject);
		noofleaders=this.getString(R.string.noofleaders);
		groupmembers=this.getString(R.string.groupmembers);
...
//		skipButton = (Button)findViewById(R.id.skipBut);
	...
	{   
		...
		if( lpp.granted == true )
		{
			...
			leader.setText(groupleader);
			errorMsg.setText(sessionexpire);
			leader.setEnabled(false);
			//skipButton.setEnabled(false);
		}
		else
...
>>>>>>> 3d5ab580ad7d05264640c490c144d6c12787b856
	...
	{
		instruction1.setText(onlinestudents+QuizAttributes.noOfOnlineStudents);
		instruction2.setText(rounds+QuizAttributes.noOfRounds);
		instruction3.setText(subject+QuizAttributes.subject);
		instruction4.setText(noofleaders+QuizAttributes.noOfLeaders+" "+groupmembers+QuizAttributes.sizeOfGroup);
	}
}
